...
import { Send, BookOpen, Target, TrendingUp, MessageSquare, CheckCircle, BarChart3, Languages, Mic, Volume2, Pause, LogOut, Library, Moon, Sun, Monitor } from 'lucide-react';
...
const LanguageTutor = () => {
  ...
  const [learningGoals, setLearningGoals] = useState([
    { id: 1, text: 'Master basic Italian greetings (Ciao, Buongiorno)', completed: false, progress: 20 },
    { id: 2, text: 'Learn present tense verbs (essere, avere)', completed: false, progress: 10 },
    { id: 3, text: 'Expand Italian food vocabulary', completed: false, progress: 0 }
  ]);
  ...
  const generateLearningGoals = (level, language) => {
    ...
    const goals = goalsByLevel[level]?.[language] || goalsByLevel.Beginner.italian;
  ...
  return (
    <div className={`flex h-screen ${isDark ? 'bg-dark-bg' : 'bg-gray-50'}`} {...tabSwipeHandlers}>
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <div className={`${isDark ? 'bg-dark-surface border-dark-border' : 'bg-white border-gray-200'} border-b p-4`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                />
                <h1 className={`text-xl font-bold ${isDark ? 'text-dark-text' : 'text-gray-800'}`}>Language Tutor</h1>
              </div>
          ...
          {/* Tab Navigation */}
          <div className={`flex border-t mt-4 -mb-4 -mx-4 px-4 ${isDark ? 'border-dark-border' : 'border-gray-200'}`}>
            <button
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-center py-8">
              ...
              <h2 className={`text-xl font-semibold mb-2 ${isDark ? 'text-dark-text' : 'text-gray-700'}`}>
                Ready to practice {languages[selectedLanguage].name}?
              </h2>
              <p className={`mb-4 ${isDark ? 'text-dark-text-secondary' : 'text-gray-500'}`}>
                Start a conversation and I'll help you learn with personalized feedback!
              </p>
              <div className={`flex items-center justify-center space-x-2 text-sm ${isDark ? 'text-dark-text-secondary' : 'text-gray-600'}`}>
        ...
        {/* Input Area */}
        <div className={`border-t p-4 ${isDark ? 'bg-dark-surface border-dark-border' : 'bg-white border-gray-200'}`}>
